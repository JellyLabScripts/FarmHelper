...
public class FarmMenu extends UIContainer {
    ...
    public static final List<Integer> FORBIDDEN_LAYERED_FARMS = Arrays.asList(2, 5, 6);
    ...
    public static void setDisable(int farmType){
        ...
        if(farmType == 0L) {
            for (UIComponent c : cropList) {
                if(FORBIDDEN_LAYERED_FARMS.contains((int)((ImageBox)c).value)) {
                    c.getChildren().get(0).setColor(DISABLED_COLOR);
                }
            }
            if(FORBIDDEN_LAYERED_FARMS.contains(FarmConfig.cropType.ordinal())) {
