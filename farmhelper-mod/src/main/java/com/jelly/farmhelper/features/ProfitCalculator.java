    ...
    public static void fetchBazaarPrices() {
        ...
        try {
            ...
            for (int i = 0; i < cropsToCount.size(); i++) {
                ...
                bazaarPrices.put(cropsToCount.get(i).localizedName, (Double) (json3).get("buyPrice")); // assume sell order
            }
            for (int i = 0; i < armorDropToCount.size(); i++) {
                JSONObject json2 = (JSONObject)json1.get(armorDropToCount.get(i).bazaarId);
                JSONObject json3 = (JSONObject)json2.get("quick_status");
                bazaarPrices.put(armorDropToCount.get(i).localizedName, (Double) (json3).get("buyPrice")); // assume sell order
                System.out.println(armorDropToCount.get(i).localizedName + " " + (Double) (json3).get("buyPrice"));
            }
