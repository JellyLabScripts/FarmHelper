...
public class AntiStuck implements IFeature {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (dx < 1 && dz < 1 && dy < 1 && !Failsafe.getInstance().isEmergency() && notMovingTimer.isScheduled() && mc.currentScreen == null) {
            if (notMovingTimer.hasPassed((long) (FarmHelperConfig.timeBetweenChangingRows + FarmHelperConfig.randomTimeBetweenChangingRows + FarmHelperConfig.antiStuckDelay)) && !Failsafe.getInstance().isTouchingDirtBlock()) {
                start();
            }
        } else {
            notMovingTimer.schedule();
