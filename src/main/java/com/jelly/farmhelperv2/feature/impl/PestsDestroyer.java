...
//          finishMacro();
      ...
      case WAIT_FOR_LOCATION:
        ...
        if (lastLocation != null && lastFireworkTime + 250 < System.currentTimeMillis()) {
          ...
          if (mc.thePlayer.getDistance(firework.xCoord, firework.yCoord, firework.zCoord) < 2) {
            ...
            LogUtils.sendWarning(
                "[Pests Destroyer] Firework is too close to player. Flying to x: " + firework.xCoord + " y: " + y + " z: " + firework.zCoord);
            break;
          }
          RotationHandler.getInstance().reset();
          state = States.FIND_PEST;
      ...
      case CHECK_ANOTHER_PEST:
        ...
        LogUtils.sendDebug(
            GameStateHandler.getInstance().getPestsCount() + " pest" + (GameStateHandler.getInstance().getPestsCount() == 1 ? "" : "s") + " left");
        if (GameStateHandler.getInstance().getPestsCount() == 0) {
          state = States.GO_BACK;
          delayClock.schedule((long) (500 + Math.random() * 500));
          break;
...
//      Multithreading.schedule(this::finishMacro, 1_000 + (long) (Math.random() * 1_000), TimeUnit.MILLISECONDS);
  ...
  public void onRender(RenderWorldLastEvent event) {
    ...
    AxisAlignedBB aabb = new AxisAlignedBB(vacuumRange.xCoord - 0.05, vacuumRange.yCoord - 0.05, vacuumRange.zCoord - 0.05, vacuumRange.xCoord + 0.05,
  ...
  private void drawESP(Entity entity) {
    AxisAlignedBB boundingBox = new AxisAlignedBB(entity.posX - 0.5, entity.posY + entity.getEyeHeight() - 0.35, entity.posZ - 0.5, entity.posX + 0.5,
    ...
    if (FarmHelperConfig.pestsESP) {
      ...
      if (distance > 5) {
        try {
          ...
          String texture = tagCompound.getCompoundTag("SkullOwner").getCompoundTag("Properties").getTagList("textures", 10).getCompoundTagAt(0)
              .getString("Value");
