    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        WaiterHandler.register(new Waiter(
                ...
                action -> {
                    ...
                    try {
                        event.getHook().sendMessageEmbeds(em).addFiles(FileUpload.fromData(Base64.decode(image), "image.png")).queue();
                    } catch (Exception e) {
                        event.getChannel().sendMessageEmbeds(em).addFiles(FileUpload.fromData(Base64.decode(image), "image.png")).queue();
                    }
                },
                timeoutAction -> {
                    try {
